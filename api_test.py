from Ced.Cogenvdecoder.CogEnvDecoder import CogEnvDecoder
import numpy as np
import cv2
...
env = CogEnvDecoder(env_name="C:/Sim2real_Win/RealGame.exe", no_graphics=False, time_scale=1, worker_id=1) # windows os
...
for i in range(num_episodes):
    ...
    for j in range(num_steps_per_episode):
        ...
        action = [0, 0.5, 0, 1]  # [vx, vy, vw, fire]; vx: the velocity at which the vehicle moves forward, vy: the velocity at which the vehicle moves to the left, vw: Angular speed of the vehicle counterclockwise rotation, fire: Shoot or not
